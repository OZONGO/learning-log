...
| `python-断言assert.md` | `-O` 剥除断言 ＋ 四类"绝不能用 assert 守"的检查（输入校验／安全检查／业务规则／数据完整性）；**试金石判据**"删掉它程序逻辑会不会变"（＋五场景判定）；**assert 是语句不是函数**（逗号 → 元组陷阱、`assert(x,"消息")` 恒真）；`-O` 的连带效应（`__debug__` 置 False → `if __debug__:` 整块蒸发，教练侧实测）；测试里用 assert 的五条理由；**"聚合同构五点"判为记录失据（留空不编）** | 2026-09-15 | 完成（正文誊自 2026-09-15 口答；第 6 问记录失据） |
