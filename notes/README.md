...
| `python-类与对象.md` | 属性查找链六步（写落实例／读走链、描述符绑定 `self`、`__qualname__` 跟定义处）；类属性共享两公式（可变＋原地改＝污染／不可变＋重贴＝不共享，`+=` 两副面孔）；`__new__` vs `__init__`；重写＝盖住≠删除、忘 super 晚炸；`__str__`/`__repr__` 回退矩阵（str→repr 有路、repr→str 没路，"repr 总是写"）；@property 校验挂存储量三理由、"无条件"的边界（`_r` 不兜底，篱笆非墙）、存算分工；七节坑录（含"订正只改对账、预测留原文"新规矩） | 2026-09-16 | 完成（正文誊自当日七问口答；八节一句话总结待收工补） |
