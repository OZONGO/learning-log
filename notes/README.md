...
| `python-断言assert.md` | `-O` 剥除断言／assert vs raise 的试金石判据／assert 是语句不是函数（逗号→元组陷阱）／`assert(x,"消息")` 反向陷阱／真实项目里 assert 的第一现场（MP1 三套测试 24／8／9） | 2026-09-15 | **脚手架就位（N013，正文待本人作答后誊入）** |
